...
export function useSuccessLogic() {
  ...
  useEffect(() => {
    async function finalize() {
      ...
      if (!raw) return router.replace("/checkout");
      ...
      if (anyWithAddress) {
        await fetch(`${process.env.NEXTAUTH_URL || ''}/api/user/address`, {
          ...
          body: JSON.stringify({
            ...parseAddress(anyWithAddress.address!),
            makeMain: true,
          }),
        });
      }
      ...
      let mainCart: LocalCheckoutItem[] = [];
      try {
        mainCart = JSON.parse(mainRaw);
      } catch {}
  ...
  return { status, invoiceNumber, transactionDate, paymentMethod, shippingMethod, items, totals } as const;
}
